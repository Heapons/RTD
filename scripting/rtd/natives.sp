...
#define GET_PERK \
	...
	if (!perk) \
		ThrowNativeError(0, "Invalid perk: %d. Is RTDPerk.Valid true?", GetNativeCell(1));
...
public int Native_Force(Handle hPlugin, int iParams)
{
	...
	GetNativeString(2, sQuery, sizeof(sQuery));
	int client = GetNativeCell(1);
	int iPerkTime = GetNativeCell(3);
	int iInitiator = GetNativeCell(4);
...
{
	int client = GetNativeCell(1);
	int iRollFlags = GetNativeCell(2);
